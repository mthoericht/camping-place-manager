...
- **Hooks** in `src/hooks/`: `use-mobile`, `useConfirmDelete`, `useCrud` (CRUD dialog + form + submit for CRUD pages), `useFetchWhenIdle`, `useOpenEditFromLocationState` (generic: open edit from `location.state`; bookings use `useOpenBookingEditFromLocationState` in the feature), `useSyncEditFormFromStore` (sync edit form with store on WS updates/deletes), `useWebSocketSync` (WebSocket connection, dispatch receive*FromWebSocket on server events)
